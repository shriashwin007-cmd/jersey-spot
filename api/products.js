  ...
  try {
    ...
    if (req.method === 'GET') {
      const rows = await sql`
        SELECT id, name, tag, category, club, price, image_url, cloudinary_public_id, sort_order, in_stock, buy_online, enquiry_clicks, created_at
        FROM products
    ...
    if (req.method === 'POST') {
      ...
      const { name, tag = '', category = 'jersey', club = '', price = 0, inStock = true, buyOnline = false, imageUrl, cloudinaryPublicId } = req.body || {};
      ...
      const [row] = await sql`
        INSERT INTO products (name, tag, category, club, price, image_url, cloudinary_public_id, sort_order, in_stock, buy_online)
        VALUES (${name}, ${tag}, ${category}, ${club}, ${sanitizePrice(price)}, ${imageUrl}, ${cloudinaryPublicId}, ${max + 1}, ${!!inStock}, ${!!buyOnline})
